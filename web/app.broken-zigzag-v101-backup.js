...
const MARK_PASS_EXTENSION_RADIUS_M = 125;
...
const COURSE_SEQUENCE = [
  { name: "bolina_1",    mode: "upwind", target: "ALFA 1",                  flipAtTarget: true,  roundRadius: 38 },
  { name: "offset_alfa2", mode: "reach",  target: "ALFA 2",                  flipAtTarget: false, roundRadius: 36 },
  { name: "gate_poppa",  mode: "run",    targets: ["ALFA 3P", "ALFA 3ST"],  flipAtTarget: true,  roundRadius: 42, gate: true },
  { name: "bolina_2",    mode: "upwind", target: "ALFA 2",                  flipAtTarget: true,  roundRadius: 38 },
  { name: "poppa_3p",    mode: "run",    target: "ALFA 3P",                 flipAtTarget: true,  roundRadius: 34 },
  { name: "arrivo",      mode: "finish", finishLine: true,                  flipAtTarget: false, roundRadius: 28 },
];
...
const DEFAULT_SCENE = {
  buoys: [
    ...
    { id: "ALFA RC",          lat: 45.8840, lon: 10.8480, kind: "committee" }, // Race committee
    { id: "ALFA PIN",         lat: 45.8840, lon: 10.8420, kind: "gate"      }, // Start/downwind pin
    { id: "ALFA FINISH",      lat: 45.8838, lon: 10.8465, kind: "finish"    }, // Committee-side finish
    { id: "ALFA FINISH END",  lat: 45.8838, lon: 10.8435, kind: "finish"    }, // Pin-side finish
    // Windward marks south of the start.
    { id: "ALFA 1",           lat: 45.8680, lon: 10.8460, kind: "pin"       }, // First windward mark
    { id: "ALFA 2",           lat: 45.8680, lon: 10.8530, kind: "pin"       }, // Reach mark / second upwind target
    // Downwind gate north of the upwind marks.
    { id: "ALFA 3P",          lat: 45.8842, lon: 10.8500, kind: "gate"      }, // Port gate mark
    { id: "ALFA 3ST",         lat: 45.8842, lon: 10.8455, kind: "gate"      }, // Starboard gate mark
  ],
  boats: [
...
function raceAuditState() {
  ...
  return {
    ...
    allStarted: boats.length > 0 && boats.every((boat) => boat.startCrossed),
    allFinished: boats.length > 0 && boats.every((boat) => boat.finished),
    boats,
...
function enforceBoatInsideArea(boat, previousPoint) {
  if (!boat || isPointInsideArea(boat)) {
    return true;
  }
  if (previousPoint && isPointInsideArea(previousPoint)) {
  ...
  const telemetry = ensureBoatTelemetry(boat);
  telemetry.note = `Course-area correction: ${boat.name ?? boat.id} was leaving the race area.`;
  return false;
}
...
function applyQwenManeuverDecision(boat, stage, target = null) {
  ...
  const memory = stageMemoryForBoat(boat);
  const stageElapsedMs = Math.max(0, state.simulatedClockMs - Number(memory.stageStartedSimMs ?? boat.race?.stageStartedAtSimMs ?? 0));
  if (isUpwindApproachSideLocked(boat, stage, target, distanceToTargetM, stageElapsedMs)) {
    decision.appliedManeuver = true;
    decision.action = "hold";
    decision.label = "Approach lock";
    decision.reason = "Mark approach lock active: external maneuver ignored until mark rounding.";
    boat.aiEngine = "approach-lock";
...
function liveBotHeading(boat, stage, target, wind) {
  ...
  const approachSideLocked = isUpwindApproachSideLocked(boat, stage, target, distanceToTargetM, stageElapsedMs);
  ...
  if ((!boat.isUserBoat || boat.demoAutopilot) && stage?.mode === "upwind" && target) {
    localDecision = chooseGardaUpwindDecision(boat, stage, target, wind);
    applyLocalGardaDecision(boat, stage, localDecision);
  ...
  const sideSign = Number(boat.liveSideSign || resolveRouteSideSign(boat));
  if (stage?.mode === "upwind") {
    const lockedSideSign = Number(boat.finalBoardLockSideSign ?? boat.finalBoardLock?.sideSign);
    const approachSideSign = Number(
      Number.isFinite(lockedSideSign) && lockedSideSign !== 0
        ? lockedSideSign
        : Number(boat.approachCommitSideSign ?? sideSign)
    );
    desiredHeading = boat.markDirectRoundingMode
      ? targetBearing
      : (boat.markApproachMode || approachCommitZone)
      ? (distanceToTargetM < 95
        ? targetBearing
        : upwindClosingHeading(boat, target, wind, approachSideSign, localDecision?.polarReliefDeg ?? 0))
      : target && distanceToTargetM < 210
      ? constrainedHeadingForStage(boat, targetBearing, wind, stage.mode)
      : allowedUpwindHeadingForSide(boat, wind, sideSign, localDecision?.polarReliefDeg ?? 0);
  } else if (stage?.mode === "reach") {
    const reachLaneAngle = clamp(BASE_REACH_LANE_ANGLE + Number(profile.reachAngleOffset ?? 0), -8, 8);
    const reachBlend = clamp(
      (distanceToTargetM - ROUTE_REACH_DIRECT_RADIUS_M) /
        Math.max(1, ROUTE_REACH_BLEND_RADIUS_M - ROUTE_REACH_DIRECT_RADIUS_M),
      0,
      1
    );
    desiredHeading = routeHeadingWithProgressGuard(
      boat,
      target,
      normalizeAngle(targetBearing + sideSign * reachLaneAngle * reachBlend),
      targetBearing,
      180
    );
  } else if (stage?.mode === "run") {
    const runLaneAngle = BASE_RUN_LANE_ANGLE + Number(profile.runAngleOffset ?? 0);
    const runBlend = clamp(
      (distanceToTargetM - ROUTE_RUN_DIRECT_RADIUS_M) /
        Math.max(1, ROUTE_RUN_BLEND_RADIUS_M - ROUTE_RUN_DIRECT_RADIUS_M),
      0,
      1
    );
    desiredHeading = routeHeadingWithProgressGuard(
      boat,
      target,
      normalizeAngle(targetBearing + sideSign * runLaneAngle * runBlend),
      targetBearing,
      170
    );
  } else if (stage?.mode === "finish") {
    const finishLaneAngle = clamp(BASE_RUN_LANE_ANGLE + Number(profile.runAngleOffset ?? 0), -12, 12);
    const finishBlend = clamp(
      (distanceToTargetM - ROUTE_FINISH_DIRECT_RADIUS_M) /
        Math.max(1, ROUTE_FINISH_BLEND_RADIUS_M - ROUTE_FINISH_DIRECT_RADIUS_M),
      0,
      1
    );
    desiredHeading = routeHeadingWithProgressGuard(
      boat,
      target,
      normalizeAngle(targetBearing + sideSign * finishLaneAngle * finishBlend),
      targetBearing,
      190
    );
  }
  ...
  const decision = boat.qwenDecision;
  if (decision && state.simulatedClockMs - Number(decision.receivedAtSimMs ?? 0) < QWEN_DECISION_INTERVAL_SIM_MS * 2) {
    if (decision.action === "head_up") {
      desiredHeading = normalizeAngle(desiredHeading - 4);
    } else if (decision.action === "bear_away") {
      desiredHeading = normalizeAngle(desiredHeading + 4);
    } else if (decision.action === "seek_pressure" && stage?.mode !== "upwind" && !approachSideLocked && decision.side && decision.side !== "center") {
      boat.liveSideSign = sideSignFromName(decision.side) || boat.liveSideSign;
    }
    desiredHeading = normalizeAngle(desiredHeading + clamp(Number(decision.heading_delta ?? 0), -QWEN_HEADING_DELTA_LIMIT_DEG, QWEN_HEADING_DELTA_LIMIT_DEG));
  }
  ...
  return stage?.mode === "upwind" && !boat.markDirectRoundingMode
...
function applyMarkRounding(boat, stage, mark = null, nextTarget = null) {
  ...
  if (exitPoint && isPointInsideArea(exitPoint)) {
    boat.lat = exitPoint.lat;
    boat.lon = exitPoint.lon;
  }
...
function movementReachedCourseStage(previousPoint, currentPoint, stage, target) {
  ...
  const radius = stage?.mode === "upwind" ? Math.max(baseRadius, 58) : (stage?.gate ? Math.max(baseRadius, 20) : Math.max(baseRadius, 46));
...
function constrainedHeadingForStage(boat, desiredHeading, wind, stageMode) {
  ...
  if (target) {
    const dist = haversineMeters(boat, target);
    if (dist < 60) {
      return normalizeAngle(desiredHeading); // Pinch/shoot the mark directly when close
    }
  }
  const profile = getBoatProfile(boat);
...
function calculateTargetSpeedKn(boat, stageMode, heading, wind, dt) {
  ...
  const targetSpeed = stageBaseSpeed * profile.speedMultiplier * profile.paceScale * wind.multiplier * polar.factor * boat.speedPenalty * recoveryFactor * dirtyAir.factor;
...
function tickBotBoatMovement(boat, dt, now) {
  ...
  boat.currentSpeedKn = Number.isFinite(Number(boat.currentSpeedKn))
    ...
    : targetSpeedKn;
  boat.heading = moveHeadingToward(Number.isFinite(Number(boat.heading)) ? boat.heading : desiredHeading, desiredHeading, profile.turnRate * dt * SIM_SPEEDUP);
  ...
  const reached = movementReachedCourseStage(previousPoint, currentPoint, stage, target);
...
function tickUserBoatMovement(boat, dt, now) {
  ...
  const wind = windEngineAt(boat, now);
  const profile = getBoatProfile(boat);
  const heading = Number.isFinite(Number(boat.heading)) ? boat.heading : initialHeadingForBoat(boat);
...
function tickSimulation() {
  ...
  if (state.courseLines.startVisible && state.boats.every((boat) => state.courseLines.startPassedBoatIds.has(boat.id))) {
    state.courseLines.startVisible = false;
  }
  if (state.courseLines.finishVisible && state.boats.every((boat) => state.courseLines.finishPassedBoatIds.has(boat.id))) {
    state.courseLines.finishVisible = false;
    state.raceStatus = "finished";
    clearInterval(state.simulationTimer);
  }
  if (state.raceStatus === "running" && state.boats.length && state.boats.every((boat) => boat.race?.finishCrossed || Number(boat.courseIndex ?? 0) >= COURSE_SEQUENCE.length)) {
    state.raceStatus = "finished";
    state.courseLines.finishVisible = false;
    clearInterval(state.simulationTimer);
  }
